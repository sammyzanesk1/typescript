...
let id = 5; //NUMBER
let company = "Traversy Media "; //STRING
let isPublished = true; //BOOLEAN AND THE BOOLEAN VALUE
let x = "Hello"; //A STATIC TYPE..a string initially...you could assign another type to any again no conflict will occur
//ARRAY TYPES
...
let arrayEmployee; //we specify here that the array should have a number type in [0] and a string in [1]in each of the arrayEmployee position or rank or nested arrays.
arrayEmployee = [
    [1, "Brad"],
    [2, "John"],
    [3, "Zane"],
...
let cid = 1; //we assign any type to variable cid
let customerId = cid; //we assign custmerId a number type by chaning the  cid to a number type.
...
//syntax 1: ANGLE BRACKET....<>
let cidA = 1; //we assign any type to variable cid
let customerIdA = cidA; //we assign custmerId a number type by chaning the  cid to a number type.
// customerId = true; //returns error, cid is a number customerId is a cid with a number type
...
console.log(userA);
const userB = { id: 1, name: "John" };
console.log(userA);
//step 2 create the variable function and the function
const add = (x, y) => x + y; //arrow function, no need to set return
const substract = (x, y) => x - y; //arrow function, no need to set return
...
let numArrayGeneric = genericgetArray([1, 2, 3, 4]); //we set the type here which replaces th function placeholder
let strArrayGeneric = genericgetArray(["King", "Sammy", "Zane"]); //we set the type here which replaces th function placeholder
